import { getAuthToken, clearAuthToken, clearRefreshToken, refreshToken } from './auth';
...
export async function graphql<T>(query: string, variables?: Record<string, any>): Promise<T> {
    const url = `http://symfony-blog.local/api/graphql`;
    ...
    if (res.status === 401) {
        try {
            ...
            if (!retryRes.ok || retryData?.errors) {
                const message = retryData?.errors?.[0]?.message || 'GraphQL request failed after token refresh';
...
async function request<T>(path: string, options: RequestInit): Promise<T> {
    ...
    if (res.status === 401) {
        try {
            ...
            const isJson = retryRes.headers.get('content-type')?.includes('application/json');
            const retryData = isJson ? await retryRes.json() : (await retryRes.text());
            if (!retryRes.ok) {
                const message = typeof retryData === 'string' ? retryData : retryData?.message || 'Request failed after token refresh';
                throw new Error(message);
            }
